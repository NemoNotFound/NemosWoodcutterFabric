...
public class WoodcutterScreen extends HandledScreen<WoodcutterScreenHandler> {
    ...
    protected void drawBackground(MatrixStack matrices, float delta, int mouseX, int mouseY) {
        ...
        this.renderRecipeBackground(matrices, mouseX, mouseY, xPosForRecipe, yPosForRecipe, scrollOffset);
        this.renderRecipeIcons(xPosForRecipe, yPosForRecipe, scrollOffset);
    }

    private void drawScroll(MatrixStack matrices) {
        int scrollOffset = (int) (41.0f * this.scrollAmount);
        drawTexture(matrices, x + 119, y + 15 + scrollOffset, 176 + (this.shouldScroll() ? 0 : 12), 0, 12, 15);
    }
    ...
    protected void drawMouseoverTooltip(MatrixStack matrices, int x, int y) {
        ...
        for (int l = this.scrollOffset; l < scrollOffset && l < this.handler.getAvailableRecipeCount(); ++l) {
            ...
            this.renderTooltip(matrices, availableRecipes.get(l)
                    .getOutput(), x, y);
        }

    }

    private void renderRecipeBackground(MatrixStack matrices, int mouseX, int mouseY, int xPosForRecipe,
    ...
    private void renderRecipeIcons(int x, int y, int scrollOffset) {
        ...
        for (int i = this.scrollOffset; i < scrollOffset && i < availableRecipes.size(); ++i) {
            ...
            this.client.getItemRenderer().renderInGuiWithOverrides(recipeEntry.getOutput(), k, m);
        }
    }
